...
void collisions() {
	...
	for (i = 3; i < 12; i++) {
		if (spritegetvalue(i, S_X) < 0)
			spritesetvalue(i, S_X, 319);
		else if (spritegetvalue(i, S_X) > 319)
			spritesetvalue(i, S_X, 0);

		if (spritegetvalue(i, S_Y) < 0)
			spritesetvalue(i, S_Y, 239);
		else if (spritegetvalue(i, S_Y) > 239)
...
void init() {
	...
	for (i = 3; i < 7; i++) {
		newObj(aster1, 20 + random(290), random(210), 16, 16, 2);
	}
...
void main() {
	...
	while (1) {
		if (spritegetvalue(1, S_X) < 0)
			spritesetvalue(1, S_X, 319);
		else if (spritegetvalue(1, S_X) > 319)
			spritesetvalue(1, S_X, 0);

		if (spritegetvalue(1, S_Y) < 0)
			spritesetvalue(1, S_Y, 239);
		else if (spritegetvalue(1, S_Y) > 239)
		...
		if ((spritegetvalue(2, S_X) < 0) || (spritegetvalue(2, S_X) > 319) 
				|| (spritegetvalue(2, S_Y) < 0) || (spritegetvalue(2, S_Y) > 239)) {
					spritesetvalue(2, S_LIVES,0);
		}
